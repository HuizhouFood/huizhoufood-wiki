    ...
    <div class="app-container py-8 sm:py-12">
      <div class="lg:grid lg:grid-cols-2 lg:items-center lg:gap-12">
        <div>
          ...
          <blockquote class="mt-6 border-l-4 border-brand-400 pl-4 italic text-slate-600">
            ...
            <footer class="mt-1 text-sm not-italic text-slate-400">— ?????????</footer>
          </blockquote>
        ...
        <div class="mt-8 lg:mt-0">
          <div :class="['app-card flex aspect-square items-center justify-center overflow-hidden bg-gradient-to-br text-9xl', product.gradient]">
            {{ product.emoji }}
          </div>
        </div>
      </div>
        ...
        <div class="mt-6 grid gap-5 lg:grid-cols-2">
          <article class="app-card p-5 sm:p-6">
            <ul class="space-y-3 text-sm leading-relaxed text-slate-600">
              <li v-for="item in product.efficacy" :key="item" class="flex gap-2">
          ...
          <article class="app-card p-5 sm:p-6">
            ...
            <ul class="mt-3 space-y-2.5 text-sm leading-relaxed text-slate-600">
              <li v-for="part in product.edibleParts" :key="part" class="flex gap-2">
      ...
      <section class="mt-12">
        ...
        <div class="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          <article
            v-for="method in product.cookingMethods"
            :key="method.title"
      ...
      <section class="mt-12">
        ...
        <div class="app-card mt-6 border border-amber-100 bg-amber-50/80 p-5">
          <ul class="space-y-2 text-sm text-amber-900">
            <li v-for="note in product.precautions" :key="note" class="flex gap-2">
